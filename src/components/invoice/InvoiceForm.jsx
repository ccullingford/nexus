...
// TODO: Later restrict editing for paid invoices or non-admin users
function canEditInvoice(invoice, user) {
...
export default function InvoiceForm({ mode = 'create', invoiceId, preselectedCustomerId }) {
  ...
  useEffect(() => {
    if (mode === 'edit' && existingInvoice) {
      ...
      setFormData({
        ...
        line_items: existingInvoice.line_items?.length > 0 
  ...
  const updateInvoiceMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      window.location.href = createPageUrl('InvoiceManagerDetail') + `?id=${invoiceId}`;
    },
  });
  ...
  const handleSubmit = async (e) => {
    ...
    if (mode === 'create') {
      ...
      updateInvoiceMutation.mutate({ id: invoiceId, data: invoiceData });
    }
  };

  if (mode === 'edit' && loadingInvoice) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-[#414257]">
            {mode === 'edit' ? 'Edit Invoice' : 'New Invoice'}
          </h1>
          <p className="text-[#5c5f7a] mt-1">
            {mode === 'edit' ? 'Update invoice details' : 'Create a new invoice manually'}
          </p>
        </div>
      </div>
