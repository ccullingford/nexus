...
import { userHasPermission } from '@/components/permissions';
import { 
  LayoutDashboard, 
  Building2, 
  FileText, 
...
const navItems = [
  ...
  { name: 'Parking Manager', icon: Car, path: 'ParkingManagerVehicles' },
  { name: 'Permits', icon: FileCheck, path: 'ParkingManagerPermits', permission: 'parking-manager:permits:view' },
...
export default function Layout({ children, currentPageName }) {
  ...
  const userDisplay = user ? {
    initials: user.full_name ? user.full_name.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase() : 'U',
        ...
        <header className="h-16 bg-white border-b border-gray-100 flex items-center justify-between px-4 lg:px-8 sticky top-0 z-30">
          ...
          <div className="flex items-center gap-3">
            <div className="hidden sm:block text-right">
              <p className="text-sm font-medium text-[#414257]">{userDisplay.name}</p>
              <p className="text-xs text-[#5c5f7a]">{userDisplay.role}</p>
            </div>
            <div className="relative group">
              ...
              <div className="absolute right-0 top-full mt-2 w-56 py-2 bg-white rounded-xl shadow-xl border border-gray-100 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                <div className="px-4 py-2 border-b border-gray-100 sm:hidden">
                  <p className="text-sm font-medium text-[#414257]">{userDisplay.name}</p>
                  <p className="text-xs text-[#5c5f7a]">{userDisplay.email}</p>
                </div>
                {user && (user.role === 'Super Admin' || user.role === 'Org Admin') && (
                  <Link 
